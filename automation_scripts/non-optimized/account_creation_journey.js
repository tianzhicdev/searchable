...
async function selectItemsInProduct(page) {
    //Select item/s
    await page.waitForSelector("div[class='MuiBox-root css-axw7ok']", elem => elem.click()); //need to change
    await page.click("div[class='MuiBox-root css-axw7ok']", elem => elem.click());
    await delay(1000);
}

// async function selectItemsInProduct(page, itemName) {
//       // Wait until the item boxes are rendered
//     await page.waitForSelector('div.MuiBox-root');

//     const ok = await page.evaluate(() => {
//         // Heuristic: a “product box” here is a MuiBox that contains
//         // a name (body1) and a price (body2 with a $)
//         const boxes = Array.from(document.querySelectorAll('div.MuiBox-root'))
//         .filter(b => {
//             const name  = b.querySelector('p.MuiTypography-body1');
//             const price = b.querySelector('p.MuiTypography-body2');
//             return name && price && /\$\s*\d/.test(price.textContent);
//         });

//         if (!boxes.length) return false;

//         const first = boxes[0];
//         // Fire a real click (some MUI components ignore plain .click())
//         const target = first.querySelector('a, button, p, div') || first;
//         target.dispatchEvent(new MouseEvent('click', { bubbles: true }));
//         return true;
//     });

//     if (!ok) throw new Error('No product item matched the heuristic.');
//     await delay(800);
// }
